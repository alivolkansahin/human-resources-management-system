...
public class JwtTokenManager {
    ...
    @Value("${comment-service-config.jwt.issuer}")
    String issuer;

    @Value("${comment-service-config.jwt.secret-key}")
    String secretKey;
    ...
    public List<String> getClaimsFromToken(String token){
        ...
        if(optionalId.isEmpty() || optionalRole.isEmpty()) throw new CommentServiceException(ErrorType.INVALID_TOKEN);
