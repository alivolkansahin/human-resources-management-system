...
public class JwtTokenManager {
    ...
    @Value("${advance-service-config.jwt.issuer}")
    String issuer;

    @Value("${advance-service-config.jwt.secret-key}")
    String secretKey;
    ...
    public List<String> getClaimsFromToken(String token){
        ...
        if(optionalId.isEmpty() || optionalRole.isEmpty()) throw new AdvanceServiceException(ErrorType.INVALID_TOKEN);
    ...
    private DecodedJWT decodeToken(String token) {
        ...
        try {
            ...
            throw new AdvanceServiceException(ErrorType.INVALID_TOKEN);
        }
    }
