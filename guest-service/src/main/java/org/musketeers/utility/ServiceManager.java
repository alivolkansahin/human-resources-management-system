...
public class ServiceManager<T extends BaseEntity,ID> implements IService<T,ID>{
    ...
    public Boolean softDeleteById(ID id) {
        Long time = System.currentTimeMillis();
        T t = mongoRepository.findById(id).orElseThrow(()-> new GuestServiceException(ErrorType.GUEST_NOT_FOUND));
        t.setStatus(false);
        t.setUpdatedAt(time);
        mongoRepository.save(t);
        return true;
    ...
    public T findById(ID id) {
        return mongoRepository.findById(id).orElseThrow(()-> new GuestServiceException(ErrorType.GUEST_NOT_FOUND));
    }
