...
public class ServiceManager<T extends BaseEntity,ID> implements IService<T,ID>{
    ...
    public Boolean softDeleteById(ID id) {
        Long time = System.currentTimeMillis();
        T t = jpaRepository.findById(id).orElseThrow(()-> new DayOffServiceException(ErrorType.DAY_OFF_NOT_FOUND));
        t.setStatus(false);
        t.setUpdatedAt(time);
        jpaRepository.save(t);
        return true;
    ...
    public T findById(ID id) {
        return jpaRepository.findById(id).orElseThrow(()-> new DayOffServiceException(ErrorType.DAY_OFF_NOT_FOUND));
    }
