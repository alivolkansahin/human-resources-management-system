...
public class JwtTokenManager {
    ...
    @Value("${spending-service-config.jwt.issuer}")
    String issuer;

    @Value("${spending-service-config.jwt.secret-key}")
    String secretKey;
    ...
    public List<String> getClaimsFromToken(String token){
        ...
        if(optionalId.isEmpty() || optionalRole.isEmpty()) throw new SpendingServiceException(ErrorType.INVALID_TOKEN);
    ...
    private DecodedJWT decodeToken(String token) {
        ...
        try {
            ...
            throw new SpendingServiceException(ErrorType.INVALID_TOKEN);
        }
    }
