...
public class JwtTokenManager {
    ...
    @Value("${personnel-service-config.jwt.issuer}")
    String issuer;

    @Value("${personnel-service-config.jwt.secret-key}")
    String secretKey;
    ...
    public List<String> getClaimsFromToken(String token){
        ...
        if(optionalId.isEmpty() || optionalRole.isEmpty()) throw new PersonnelServiceException(ErrorType.INVALID_TOKEN);
    ...
    private DecodedJWT decodeToken(String token) {
        ...
        try {
            ...
            throw new PersonnelServiceException(ErrorType.INVALID_TOKEN);
        }
    }
